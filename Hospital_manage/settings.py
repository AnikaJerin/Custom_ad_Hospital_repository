...
class HospitalSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].set_param('Hospital_manage.note', self.note)
        print("test", self.product_ids.ids)
        self.env['ir.config_parameter'].set_param('Hospital_manage.product_ids', self.product_ids.ids)
        return res
    ...
    def get_values(self):
        ...
        notes = ICPSudo.get_param('Hospital_manage.note')
        product_ids = self.env['ir.config_parameter'].sudo().get_param('Hospital_manage.product_ids')
        if product_ids:
